...
class CarrierServices
{
    ...
    public function saveTariffs(string $carrierCode, array $data): bool
    {
        ...
        if (empty($carrier)) {
            PrestaShopLogger::addLog('Carrier non trovato', 3);
            return false;
        }

        $idCarrier = (int) $carrier['id_carrier'];
        $serviceCode = pSQL($carrier['service_code']);

        // validazione
        try {
            $this->validateRows($data, $serviceCode);
        } catch (Exception $e) {
        ...
        try {
            ...
            foreach ($data as $row) {
                ...
                $tariff = (float) str_replace(',', '.', $row['tariff'] ?? 0);
